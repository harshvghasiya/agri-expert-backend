        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ trans('message.index_question_breadcrum') }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="row m-4">
                <div class="col-md-6">
                    <div class="btn-group">
                        <a href="{{ route('admin.question.create') }}" id="sample_editable_1_new" class="btn btn-primary">
                ...
                <div class="col-md-6">
                    <div class="btn-group float-end mx-1">
                        <button type="button" class="btn btn-info text-light"
                            onclick="statusAll('con_check','{{ route('admin.question.status_all') }}',1)">{{ trans('message.acitve_all') }}</button>
                    </div>
                    <div class="btn-group float-end">
                        <button type="button" class="btn btn-warning text-light"
                            onclick="statusAll('con_check','{{ route('admin.question.status_all') }}',2)">{{ trans('message.inacitve_all') }}</button>
                    </div>
                    <div class="btn-group float-end mx-1">
                        <a class="btn btn-danger "
                            onclick="deleteAll('con_check','{{ route('admin.question.delete_all') }}')">{{ trans('message.delete_all_label') }}
                        </a>
                    </div>
                </div>
            </div>
            <div class="table-responsive">
                ...
                <table id="question-table" class="table table-striped" style="width:100%">
                    <thead>
                        <tr>
                            <th><input type="checkbox" name="checkbox[]" class="con_check" value="1" id="select_all" />
                            </th>
                            <th>{{ trans('message.question') }}</th>
                            <th>{{ trans('message.image') }}</th>
                            <th>{{ trans('message.video') }}</th>
                            <th>{{ trans('message.audio') }}</th>
                            <th>{{ trans('message.upvoted') }}</th>
                            <th>{{ trans('message.asked_by') }}</th>
...
@section('script')
    ...
    <script>
        var oTable = $('#question-table').DataTable({
            ...
            responsive: true,
            ajax: {
                url: '{!! route('admin.question.any_data') !!}',
                data: function(d) {
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "{{ route('admin.question.single_status_change') }}",
                                data: {
                                    "status": status,
                                    "id": id,
                                    "_token": "{{ csrf_token() }}"
