    ...
    <div class="card">
        <div class="card-body">
            <div class="row m-4">
                <div class="col-md-6">
                    <div class="btn-group">
                        <a href="{{ route('admin.email_template.create') }}" id="sample_editable_1_new" class="btn btn-primary">
                ...
                <div class="col-md-6">
                    <div class="btn-group float-end mx-1">
                        <button type="button" class="btn btn-info text-light"
                            onclick="statusAll('con_check','{{ route('admin.email_template.status_all') }}',1)">{{ trans('message.acitve_all') }}</button>
                    </div>
                    <div class="btn-group float-end">
                        <button type="button" class="btn btn-warning text-light"
                            onclick="statusAll('con_check','{{ route('admin.email_template.status_all') }}',2)">{{ trans('message.inacitve_all') }}</button>
                    </div>
                </div>
            </div>
            <div class="table-responsive">
                ...
                <table id="email_template-table" class="table table-striped" style="width:100%">
                    <thead>
                        <tr>
...
@section('script')
    ...
    <script>
        var oTable = $('#email_template-table').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            responsive: true,
            ajax: {
                url: '{!! route('admin.email_template.any_data') !!}',
                data: function(d) {
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "{{ route('admin.email_template.single_status_change') }}",
                                data: {
                                    "status": status,
                                    "id": id,
                                    "_token": "{{ csrf_token() }}"
