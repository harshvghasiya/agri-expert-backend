        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ trans('message.index_query_breadcrum') }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="row m-4">
                <div class="col-md-6">
                    <div class="btn-group">
                        <a href="{{ route('admin.query.create') }}" id="sample_editable_1_new" class="btn btn-primary">
                ...
                <div class="col-md-6">
                    <div class="btn-group float-end mx-1">
                        <button type="button" class="btn btn-info text-light"
                            onclick="statusAll('con_check','{{ route('admin.query.status_all') }}',1)">{{ trans('message.acitve_all') }}</button>
                    </div>
                    <div class="btn-group float-end">
                        <button type="button" class="btn btn-warning text-light"
                            onclick="statusAll('con_check','{{ route('admin.query.status_all') }}',2)">{{ trans('message.inacitve_all') }}</button>
                    </div>
                    <div class="btn-group float-end mx-1">
                        <a class="btn btn-danger "
                            onclick="deleteAll('con_check','{{ route('admin.query.delete_all') }}')">{{ trans('message.delete_all_label') }}
                        </a>
                    </div>
                </div>
            </div>
            <div class="table-responsive">
                ...
                <table id="query-table" class="table table-striped" style="width:100%">
                    <thead>
                        <tr>
...
@section('script')
    ...
    <script>
        var oTable = $('#query-table').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            responsive: true,
            ajax: {
                url: '{!! route('admin.query.any_data') !!}',
                data: function(d) {
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "{{ route('admin.query.single_status_change') }}",
                                data: {
                                    "status": status,
                                    "id": id,
                                    "_token": "{{ csrf_token() }}"
